...
for DIR in `ls -d results-*`; do IN=`echo $DIR | cut -d '-' -f 2-`;
	...
	echo "<h2 style=\"padding: 10px; background: #DDDDDD; width: 100%\">fastSTRUCTURE, $PARMS, $IN</h2>" > $HTMLOUTFILE;
	for K in 1 2 3 4 5 6 7; do
		FILESTUB="p$P-r$R-$IN-K$K";
		montage -tile 1x -geometry 200 results-$IN/rep*/*.$K.svg results-$IN/$FILESTUB.html;
